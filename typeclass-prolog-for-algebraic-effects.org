...
a way to combine these into a new functor \( * -> * \). This is what this type
does.
...
turn an src_haskell{f x} into a src_haskell{fs x} whenever src_haskell{f :∈:
fs}.
...
The empty set which is a subset of everything (this is the base case of our
typeclass recursion).
...
necessary code to rearance the order of a functor sum.
...
free monad on the sum of a list of effects, except it forces the 
...
we gave out. We would like to remember some state during our traversal of the
effect tree. A src_haskell{FoldHandler} lets us do that.
